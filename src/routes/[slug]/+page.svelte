<script lang="ts">
	...
	async function checkUrlStatus(slug: string) {
		try {
			...
			const response = await fetch(`/api/urls/check/${slug}`);
			...
			// Handle successful response (non-password-protected URL)
			if (response.ok) {
				const data = await response.json();
				// Track the click first
				await trackClick(data.urlId);
				// Then redirect
			...
			if (response.status === 404) {
				...
				error = 'URL is inactive or has expired';
			} else {
				const data = await response.json();
	...
	async function trackClick(urlId: string) {
		try {
			await fetch(`/api/urls/${urlId}/clicks`, {
				method: 'POST',
				headers: {
